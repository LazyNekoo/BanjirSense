...
import { Camera, RefreshCw, Send, Moon, SkipForward, Image as Gallery } from 'lucide-react';
...
export const SOSCameraCapture = ({
  ...
  // ✅ NEW: track when video is actually ready
  ...
  // Initialize camera when video node mounts
  ...
  //for user to pick existing photo instead of taking new one
  ...
  const handleFileSelected = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = () => {
      ...
      setCapturedImage(dataUrl);  // ✅ reuse existing flow
      stopCamera();               // ✅ stop camera (optional)
    };
    reader.readAsDataURL(file);

    // reset value so user can re-upload same file again
    e.target.value = "";
  };
  ...
  return (
    <div className="min-h-screen bg-zinc-900 flex items-center justify-center p-0 md:p-4 font-display">
      <div className="w-full max-w-[400px] h-[824px] bg-zinc-900 rounded-[3rem] shadow-2xl overflow-hidden flex flex-col relative border border-zinc-700 font-display">
         {/* ✅ Hidden file input (must be inside return) */}
      <input
        ref={fileInputRef}
        type="file"
        accept="image/*"
      ...
      />
        ...
        <header className="relative z-20 flex-none px-6 pt-8 pb-4 flex justify-between items-center border-b border-zinc-800/50">
          <div className="flex items-center gap-2">
            ...
            <span className="font-bold text-lg tracking-tight text-white">Capture Situation</span>
          </div>
        ...
        <div className="relative z-20 flex justify-center pt-4 pb-2">
          <div className="flex items-center gap-4 bg-black/60 backdrop-blur-md px-4 py-1.5 rounded-full border border-white/10">
            <div className="flex items-center gap-1.5">
              <span className={`w-2 h-2 rounded-full ${isGpsActive ? 'bg-green-500 animate-pulse' : 'bg-zinc-600'}`} />
        ...
        <main className="relative z-10 flex-1 flex flex-col">
          <div className="relative flex-1 mx-4 my-2 rounded-2xl overflow-hidden bg-zinc-800 border border-zinc-700">
            {!capturedImage ? (
              <>
                ...
                {/* ✅ IMPORTANT: hidden canvas exists now */}
                <canvas ref={canvasRef} className="hidden" />

                {/* ✅ Show placeholder ONLY when camera not ready */}
                {!cameraReady && (
              ...
              <>
                <img src={capturedImage} alt="Captured situation" className="w-full h-full object-cover" />
                <canvas ref={canvasRef} className="hidden" />
              </>
            )}
          ...
          <div className="px-6 pt-3 pb-2 text-center">
            <p className="text-white/60 text-[11px] font-bold uppercase tracking-widest">
              {!capturedImage ? 'Take a clear photo of rising water or obstacles' : 'Review your capture before sending'}
            </p>
          </div>
        </main>
          ...
          {!capturedImage ? (
            <div className="flex flex-col items-center gap-4">
              <div className="relative flex items-center justify-center">
                ...
                {/* Camera + Ring wrapper */}
                <div className="relative flex items-center justify-center">
                  {/* Outer ring aligned to camera only */}
                  ...
                  <button
                    ...
                    style={{ boxShadow: '0 0 50px 10px rgba(239, 68, 68, 0.4)' }}
                    title="Capture photo"
                  >
                ...
                <button
                  ...
                  className="w-14 h-14 rounded-2xl bg-white/5 border border-white/15 text-white flex items-center justify-center hover:bg-white/10 active:bg-white/15 transition-colors"
                  title="Upload from gallery"
                >
                  <Gallery className="w-6 h-6 text-white/80" />
                </button>

              </div>
              </div>
              <p className="text-white/40 text-[10px] font-bold uppercase tracking-widest">Center hazard in frame</p>
            </div>
          ) : (
            <div className="w-full flex flex-col items-center gap-3">
              <div className="w-full flex gap-3">
                ...
                <button
                  ...
                  style={{ boxShadow: '0 0 30px 5px rgba(239, 68, 68, 0.3)' }}
                >
