...
interface MedicalProfileSuccessScreenProps {
  onNavigate?: (screen: string) => void;
  onRedirectComplete?: () => void;
  redirectDelay?: number;
}

export function MedicalProfileSuccessScreen({
  onNavigate,
  onRedirectComplete,
  redirectDelay = 3000,
}: MedicalProfileSuccessScreenProps) {
  useEffect(() => {
    ...
    <div className="min-h-screen bg-slate-100 flex items-center justify-center p-0 md:p-4 font-display text-dark-text">
      <div className="w-[400px] h-[824px] bg-white rounded-[3rem] shadow-2xl overflow-hidden flex flex-col relative border border-slate-200">
        <div className="flex-1 flex flex-col items-center justify-center px-8 text-center">
          ...
          <h1 className="text-2xl font-bold text-slate-900 mb-4">Medical Profile Updated!</h1>
          <p className="text-slate-600 leading-relaxed mb-12 max-w-xs">
            Your critical health data is now securely synced with your rescue payload for emergency responders.
          </p>
...
export default MedicalProfileSuccessScreen;
